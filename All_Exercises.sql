...
values ('0-1579-4703-3', 'Yellow Ink Tears: A Collection of Sweats', 'book about poems written by yellow ink', 4.5,
        'English');
...
INSERT INTO book(isbn, title, description, rating, language)
VALUES ('0593135202', 'Project Hail Mary',
        'All he knows is that he’s been asleep for a very, very long time. And he’s just been awakened to find himself millions of miles from home, with nothing but two corpses for company.',
        4.7, 'English'),
       ('979-8376385708', 'War Head',
        ...
        4.7, 'English'),
       ('9780143111580', 'Dune (Penguin Galaxy',
...
-- adding orders
select *
from make_order_customer_book('0241512425', create_order_for_customer(5));
-- using 44 here because we know what order was created
select *
from make_order_customer_book('0241512425', 45);
select *
from make_order_customer_book('0241512425', 45);
select *
from make_order_customer_book('9781338801910', 45);
select *
from make_order_customer_book('9781338801910', 45);
...
VALUES ('9780945774259', 'Tales from two pockets', null, 4.5, 'English'),
       ('9780486419268', 'R.U.R',
        'One of the classics of twentieth-century theater, this brilliantly conceived and exquisitely executed play by Czech playwright Karel Čapek (1890–1938) looks to a future in which all workers are automatons, or "robots" — a word this play made a permanent part of the language.',
        4.6, 'English'),
...
SELECT *
FROM book
WHERE isbn in (SELECT bookid
               from authorbooks
               where authorid = (SELECT id from author where firstname = 'Karel' and lastname = 'Čapek'));
...
SELECT create_order_for_customer((SELECT id
                                  from customer
                                  where firstname = 'Tanja'
                                    and lastname = 'Shriram'
                                    and city = 'Holbæk'
                                    and zipcode = '4300'
                                    and country = 'Denmark'
                                    and street = 'Albanivej'
                                    and streetnumber = '17'));
SELECT create_order_for_customer((SELECT id
                                  from customer
                                  where firstname = 'Tanja'
...
UPDATE itemorder
SET totalprice = (SELECT SUM(price) sumPrice
                  from type
                           inner join itemordertype i on type.id = i.typeid
                  where orderid = 1)
WHERE id = 45;
...
SELECT AVG(printlength) as average,
       id               AS avg_column1
FROM (
         SELECT printlength, id
         FROM kindle
         UNION ALL
         SELECT printlength, id
         FROM paperback
...
select number_of_books_sold, title
from books_were_sold
...
SELECT b.isbn, b.title, b.description, b.rating, b.language
from book b
         join type t on b.isbn = t.bookid
         join itemordertype i on t.id = i.typeid
group by b.isbn, b.title, b.description, b.rating, b.language
order by count(i.typeid) desc
limit 10;
...
SELECT g.name
from genre g
         join bookgenre bg on g.id = bg.genreid
         join type t on t.bookid = bg.bookid
         join itemordertype i on t.id = i.typeid
group by g.name
order by count(i.typeid) desc
limit 3;
